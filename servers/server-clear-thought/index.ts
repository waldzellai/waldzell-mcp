...
export default function({ sessionId, config }: { sessionId: string, config?: any }) {
  ...
  sessionServer.tool(
    ...
    'Apply mental models to analyze problems systematically',
    {
      modelName: z.string().describe('Name of the mental model being applied'),
      problem: z.string().describe('The problem or situation to analyze'),
      steps: z.array(z.string()).describe('Step-by-step application of the mental model'),
      reasoning: z.string().describe('The reasoning process and insights'),
      conclusion: z.string().describe('Conclusions drawn from applying the mental model')
    },
    async (args) => {
      return {
        content: [{
  ...
  sessionServer.tool(
    ...
    'Apply systematic debugging approaches to identify and resolve issues',
    {
      approachName: z.string().describe('Name of the debugging approach being used'),
      issue: z.string().describe('Description of the issue or problem'),
      steps: z.array(z.string()).describe('Systematic steps taken to debug the issue'),
      findings: z.string().describe('What was discovered during the debugging process'),
      resolution: z.string().describe('How the issue was resolved or next steps')
    },
    async (args) => {
      return {
        content: [{
    ...
    async (args) => {
      return {
        content: [{
          ...
          text: JSON.stringify({
            topic: args.topic,
            personasCount: args.personas.length,
            contributionsCount: args.contributions.length,
            stage: args.stage,
            iteration: args.iteration,
            nextContributionNeeded: args.nextContributionNeeded,
      ...
      return {
        content: [{
          ...
          text: JSON.stringify({
            ...
            stage: args.stage,
            overallConfidence: args.overallConfidence,
            uncertaintyAreasCount: args.uncertaintyAreas.length,
