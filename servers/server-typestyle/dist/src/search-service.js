...
export class SearchService {
    ...
    constructor(config) {
        ...
        this.primaryClient = new McpClient(config.primaryMcpServer.url, config.primaryMcpServer.token);
        // Initialize fallback clients if any
        if (config.fallbackMcpServers && config.fallbackMcpServers.length > 0) {
            this.fallbackClients = config.fallbackMcpServers.map(serverConfig => new McpClient(serverConfig.url, serverConfig.token));
        }
    }
    /**
    ...
    async search(query) {
        ...
        try {
            const response = await this.searchWithServer(this.primaryClient, this.config.primaryMcpServer.toolName, query);
     ...
     */
    async tryFallbackServers(query) {
        for (let i = 0; i < this.fallbackClients.length; i++) {
            try {
                const fallbackConfig = this.config.fallbackMcpServers?.[i];
                if (!fallbackConfig)
                    continue;
                const response = await this.searchWithServer(this.fallbackClients[i], fallbackConfig.toolName, query);
                // Cache result if successful and caching is enabled
                if (this.config.cacheResults && !response.isError) {
                    this.cacheResponse(query, response);
                }
     ...
     */
    async searchWithServer(client, toolName, query) {
    ...
    parseSearchResponse(response, query) {
        ...
        try {
            ...
            if (Array.isArray(parsedContent) &&
                ...
                return {
                    results: parsedContent.map((item) => ({
