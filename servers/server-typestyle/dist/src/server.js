 ...
 */
import { STYLE_CATEGORIES, DEFAULT_CONFIG } from './constants.js';
...
export class TypeStyleServer {
     ...
     */
    async processStyleRequest(input) {
        try {
            ...
            try {
                ...
                let searchQuery = '';
                if (request.code) {
                    searchQuery = this.queryGenerator.generateQueryFromCode(request.code, request.category);
                }
    ...
    analyzeStyle(request) {
        ...
        if (request.rule) {
            response.explanation = this.getRuleExplanation(request.rule);
        ...
        if (request.query) {
            ...
            if (query.includes('naming') || query.includes('name')) {
                ...
                response.recommendations = Object.values(STYLE_CATEGORIES).map(cat => `Ask about ${this.getCategoryDisplayName(cat)} rules`);
            }
        }
        return response;
    }
     ...
     */
    formatOutputForConsole(response) {
        let title = '📘 TypeStyle';
...
│ Authoritative Sources:${' '.repeat(border.length - 22)} │`;
            response.authoritative.forEach((auth) => {
...
└${border}┘`;
    ...
    checkPerformanceOptimizations(code) {
        const feedback = [];
