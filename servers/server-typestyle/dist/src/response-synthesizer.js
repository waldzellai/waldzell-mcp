...
export class ResponseSynthesizer {
     ...
     */
    combineResults(styleAnalysis, searchResponse) {
        ...
        const relevantResults = this.extractRelevantResults(searchResponse.results, styleAnalysis.category || '');
        ...
        enhancedResponse.groundedExplanation = this.createGroundedExplanation(styleAnalysis, relevantResults);
        // Enhance the feedback with authoritative context
        if (styleAnalysis.feedback && styleAnalysis.feedback.length > 0) {
            enhancedResponse.feedback = this.enhanceFeedbackWithGrounding(styleAnalysis.feedback, relevantResults);
        }
        return enhancedResponse;
    }
    /**
     ...
     */
    extractRelevantResults(results, category) {
     ...
     */
    createGroundedExplanation(styleAnalysis, relevantResults) {
     ...
     */
    enhanceFeedbackWithGrounding(feedback, searchResults) {
     ...
     */
    findMostRelevantResult(feedbackItem, searchResults) {
     ...
     */
    extractCitation(content, feedbackItem) {
