...
export class StyleGuideQueryGenerator {
     ...
     */
    generateQueryFromCode(code, category) {
     ...
     */
    getUrlForPattern(label, context) {
        ...
        const categoryKey = Object.entries(STYLE_CATEGORIES).find(([_, value]) => value.toLowerCase().includes(label.toLowerCase()))?.[1];
    ...
    extractKeyTerms(query) {
        ...
        const keyTerms = tokens.filter(token => !commonWords.includes(token) &&
            token.length > 2 &&
            // Remove punctuation
     ...
     * @returns Optimized search query
     */
    generateCategorySpecificQuery(category, patterns) {
        const categoryDisplay = this.getCategoryDisplayName(category);
        // Find patterns related to this category
        const relevantPatterns = patterns.filter(p => p.label.toLowerCase().includes(category.toLowerCase()) ||
    ...
    getRuleDisplayName(rule) {
        // Convert from snake_case to readable format
        const words = rule.split('_').map(word => word.charAt(0).toUpperCase() + word.slice(1));
        return words.join(' ');
    }
}
